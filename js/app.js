...
(function () {
  ...
  async function load() {
    try {
      ...
      if (DataStore.listDocumentos) documentos = await DataStore.listDocumentos();
      if (DataStore.listItens) itens = await DataStore.listItens();
    } catch (e) {
      console.error(e);
    ...
    populateDatalists();
    render();
    if ($("docsList")) renderDocumentosTab();
    maybeOpenFromHash();
  }

  /* ---------- Helpers de itens (árvore) ------------------------------ */
  function itemSort(a, b) {
    if (a.ordem != null && b.ordem != null && a.ordem !== b.ordem) return a.ordem - b.ordem;
    return String(a.codigo || a.titulo || "").localeCompare(
      String(b.codigo || b.titulo || ""), undefined, { numeric: true }
    );
  }
  function itemLabel(n) {
    return [n.codigo, n.titulo].filter(Boolean).join(" — ") || "(sem título)";
  }
  function itemTreeRoots(docId) {
    const docItens = itens.filter((i) => i.documento_id === docId);
    const byId = {};
    docItens.forEach((i) => { byId[i.id] = { ...i, children: [] }; });
    const roots = [];
  ...
  function renderDocumentosTab() {
    ...
    if (!documentos.length) {
      wrap.innerHTML = `<div class="empty"><div class="empty-icon">📄</div><h3>Nenhum documento cadastrado</h3></div>`;
      return;
    }
    wrap.innerHTML = documentos.slice().sort((a, b) =>
    ...
    <div class="doc-block-head" data-open-doc="${txt(d.id)}">
      <div>
        <div class="doc-block-nome">${txt(d.nome)}</div>
        <div class="doc-block-meta">${d.sigla ? txt(d.sigla) + " · " : ""}${d.orgao ? txt(d.orgao) + " · " : ""}${its.length} item(ns) · ${qn} pergunta(s)</div>
      </div>
  ...
  async function openDocDetailModal(docId) {
    ...
    if (!doc) return;
    $("dModalTitle").textContent = doc.nome;
    $("dModalMeta").textContent = [doc.sigla, doc.tipo, doc.orgao, doc.ano].filter(Boolean).join(" · ");
  ...
  function renderDocModalTree() {
    ...
    const hasAnyParent = Object.values(byId).some((n) => n.children.length > 0);
    const tree = $("dModalTree"), empty = $("dModalEmpty"), colBar = $("dModalColBar");
    if (!docItens.length) { tree.innerHTML = ""; empty.hidden = false; colBar.hidden = true; return; }
    empty.hidden = true;
    colBar.hidden = !hasAnyParent;
    const flat = [];
    function walk(n, depth) {
      flat.push({ n, depth, hasChildren: n.children.length > 0 });
      if (!docCollapsed.has(n.id)) n.children.slice().sort(itemSort).forEach((c) => walk(c, depth + 1));
    }
    roots.slice().sort(itemSort).forEach((n) => walk(n, 0));
    tree.innerHTML = flat.map(({ n, depth, hasChildren }) => {
      const collapsed = docCollapsed.has(n.id);
      const open = hasChildren && !collapsed;
      const qCount = records.filter((r) => r.item_id === n.id).length;
      const qBadge = qCount > 0
        ? `<span class="item-qcount has-q">${qCount} perg.</span>`
        : `<span class="item-qcount no-q">sem perguntas</span>`;
      const chevron = hasChildren
        ? `<button class="dd-toggle" data-id="${txt(n.id)}" aria-expanded="${open}" title="${collapsed ? "Expandir" : "Recolher"}"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="transform:rotate(${open ? 0 : -90}deg);transition:transform .2s"><path d="m6 9 6 6 6-6"/></svg></button>`
        : `<span class="dd-toggle-spacer"></span>`;
      return `<div class="dd-item" data-depth="${Math.min(depth, 4)}" style="margin-left:${depth * 22}px">
      ${chevron}
      ${n.codigo ? `<span class="item-codigo">${txt(n.codigo)}</span>` : ""}
      <span class="item-titulo">${txt(n.titulo || "")}</span>
      ${qBadge}
      <div class="item-actions">
        <button class="row-btn" data-edit-item="${txt(n.id)}" title="Editar item"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4Z"/></svg></button>
        <button class="row-btn danger" data-del-item-modal="${txt(n.id)}" title="Excluir item"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6M10 11v6M14 11v6M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg></button>
      </div>
    </div>`;
    }).join("");
    tree.querySelectorAll(".dd-toggle").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (docCollapsed.has(id)) docCollapsed.delete(id); else docCollapsed.add(id);
        renderDocModalTree();
      });
    });
  }
  ...
  function openEditDocModal(doc) {
    $("edNome").value = doc.nome || "";
    $("edSigla").value = doc.sigla || "";
    $("edTipo").value = doc.tipo || "";
    $("edOrgao").value = doc.orgao || "";
    $("edAno").value = doc.ano || "";
    $("editDocModal").dataset.editId = doc.id;
    $("editDocModal").hidden = false;
    setTimeout(() => $("edNome").focus(), 60);
  }

  async function saveEditDoc() {
    const id = $("editDocModal").dataset.editId;
    const nome = $("edNome").value.trim();
    if (!nome) { toast("Informe o nome do documento.", "error"); $("edNome").focus(); return; }
    try {
      await DataStore.updateDocumento(id, {
        nome,
        sigla: $("edSigla").value.trim() || null,
        tipo: $("edTipo").value.trim() || null,
        orgao: $("edOrgao").value.trim() || null,
        ano: $("edAno").value ? parseInt($("edAno").value, 10) : null,
      });
      toast("Documento atualizado.", "success");
      $("editDocModal").hidden = true;
      await load();
      if (currentModalDocId === id && !$("docDetailModal").hidden) {
  ...
  function openEditItemModal(item) {
    ...
    $("editItemModal").dataset.editId = item.id;
    $("editItemModal").hidden = false;
    setTimeout(() => $("eiCodigo").focus(), 60);
  }

  async function saveEditItem() {
    const id = $("editItemModal").dataset.editId;
    const codigo = $("eiCodigo").value.trim();
    const titulo = $("eiTitulo").value.trim();
    if (!codigo && !titulo) { toast("Informe código ou título.", "error"); return; }
    try {
      await DataStore.updateItem(id, { codigo: codigo || null, titulo: titulo || null });
  ...
  /* ---------- Adicionar item (a partir do modal) --------------------- */
  function openAddItemModal() {
    $("aiCodigo").value = "";
    $("aiTitulo").value = "";
    fillAddItemParentSelect();
    $("addItemModal").hidden = false;
    setTimeout(() => $("aiCodigo").focus(), 60);
  }

  function fillAddItemParentSelect() {
    const sel = $("aiParent");
    let opts = `<option value="">— Raiz —</option>`;
    const walk = (n, d) => {
      opts += `<option value="${txt(n.id)}">${"  ".repeat(d)}${txt(itemLabel(n))}</option>`;
      n.children.slice().sort(itemSort).forEach((c) => walk(c, d + 1));
    };
    if (currentModalDocId) itemTreeRoots(currentModalDocId).sort(itemSort).forEach((n) => walk(n, 0));
    sel.innerHTML = opts;
  }

  async function saveAddItem() {
    const codigo = $("aiCodigo").value.trim();
    const titulo = $("aiTitulo").value.trim();
    if (!codigo && !titulo) { toast("Informe código ou título.", "error"); return; }
    try {
      await DataStore.createItem({
        documento_id: currentModalDocId,
        parent_id: $("aiParent").value || null,
        codigo: codigo || null,
        titulo: titulo || null,
        ordem: itens.filter((i) => i.documento_id === currentModalDocId).length,
      });
      toast("Item adicionado.", "success");
      $("addItemModal").hidden = false; // keep open for adding more? No, close it
  ...
  function bind() {
    ...
    if ($("docsList")) {
      $("docsList").addEventListener("click", async (e) => {
        ...
        if (openBtn && !openBtn.hasAttribute("data-del-doc")) {
          e.stopPropagation();
          await openDocDetailModal(openBtn.dataset.openDoc);
          return;
        }
        ...
        if (delBtn) {
          ...
          const doc = documentos.find((d) => d.id === id);
          if (doc && await confirmDialog("Excluir documento", `Excluir "${doc.nome}" e todos seus itens?`, "Excluir")) {
            try { await DataStore.removeDocumento(id); toast("Documento excluído.", "success"); await load(); }
    ...
    // Modal detalhe do documento
    $("dModalClose").addEventListener("click", closeDocDetailModal);
    $("docDetailModal").addEventListener("click", (e) => { if (e.target === $("docDetailModal")) closeDocDetailModal(); });
    $("dModalEditBtn").addEventListener("click", () => {
      const doc = documentos.find((d) => d.id === currentModalDocId);
      if (doc) openEditDocModal(doc);
    });
    $("dModalDeleteBtn").addEventListener("click", async () => {
      const doc = documentos.find((d) => d.id === currentModalDocId);
      if (!doc) return;
      if (await confirmDialog("Excluir documento", `Excluir "${doc.nome}" e todos seus itens?`, "Excluir")) {
        try { await DataStore.removeDocumento(doc.id); toast("Documento excluído.", "success"); closeDocDetailModal(); await load(); }
        catch (e) { toast("Erro: " + (e.message || e), "error"); }
      }
    });
    $("dModalExpandAll").addEventListener("click", () => { docCollapsed.clear(); renderDocModalTree(); });
    $("dModalCollapseAll").addEventListener("click", () => {
      itens.filter((i) => i.documento_id === currentModalDocId).forEach((i) => {
        if (itens.some((c) => c.parent_id === i.id)) docCollapsed.add(i.id);
      });
      renderDocModalTree();
    });
    $("dModalAddItemBtn").addEventListener("click", openAddItemModal);
    $("dModalTree").addEventListener("click", async (e) => {
      const editBtn = e.target.closest("[data-edit-item]");
      if (editBtn) { const item = itens.find((i) => i.id === editBtn.dataset.editItem); if (item) openEditItemModal(item); return; }
      const delBtn = e.target.closest("[data-del-item-modal]");
      if (delBtn) {
        const id = delBtn.dataset.delItemModal;
        const item = itens.find((i) => i.id === id);
        if (item && await confirmDialog("Excluir item", `Excluir "${itemLabel(item)}"?`, "Excluir")) {
          try { await DataStore.removeItem(id); toast("Item excluído.", "success"); await load(); renderDocModalTree(); }
          catch (e) { toast("Erro: " + (e.message || e), "error"); }
        }
        return;
      }
    ...
    // Editar documento
    $("editDocClose").addEventListener("click", () => ($("editDocModal").hidden = true));
    $("editDocCancel").addEventListener("click", () => ($("editDocModal").hidden = true));
    $("editDocSave").addEventListener("click", saveEditDoc);
    $("editDocModal").addEventListener("click", (e) => { if (e.target === $("editDocModal")) $("editDocModal").hidden = true; });
    $("edNome").addEventListener("keydown", (e) => { if (e.key === "Enter") { e.preventDefault(); saveEditDoc(); } });

    // Editar item
    $("editItemClose").addEventListener("click", () => ($("editItemModal").hidden = true));
    $("editItemCancel").addEventListener("click", () => ($("editItemModal").hidden = true));
    $("editItemSave").addEventListener("click", saveEditItem);
    $("editItemModal").addEventListener("click", (e) => { if (e.target === $("editItemModal")) $("editItemModal").hidden = true; });

    // Adicionar item
    $("addItemClose").addEventListener("click", () => ($("addItemModal").hidden = true));
    $("addItemCancel").addEventListener("click", () => ($("addItemModal").hidden = true));
    $("addItemSave").addEventListener("click", saveAddItem);
